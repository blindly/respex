...
func TestBuildEmptyBinary(t *testing.T) {
	_, _, err := Build([]string{""}, DeliveryStdin, "p", "s")
	if err == nil || !strings.Contains(err.Error(), "element 0 (binary) is empty") {
		t.Fatalf("err = %v", err)
	}
}
...
func TestBuildStdinSpecPath(t *testing.T) {
	...
	if string(stdin) != "p" || strings.Join(cmd.Args, " ") != "agent /r/S.md" {
		t.Fatalf("cmd = %v, stdin = %q", cmd.Args, stdin)
	}
}
```
...
- [ ] **Step 3: Implement Build (create `internal/agent/adapter.go`)**

`internal/agent/adapter.go` is a NEW file in the same package — it must NOT redeclare `package agent`, the `Expand` function, or the placeholder constants (all already in prompts.go from Task 7). Imports needed: `errors`, `fmt`, `os/exec`, `regexp`, `strings`.
...
func Build(tpl []string, delivery, prompt, specPath string) (*exec.Cmd, []byte, error) {
	...
	if tpl[0] == "" {
		return nil, nil, errors.New(
			"agent: command element 0 (binary) is empty — set [agent] command in .respex/config.toml")
	}
	...
	default:
		return nil, nil, fmt.Errorf("agent: unknown delivery %q; want \"argv\" or \"stdin\"", delivery)
	}
	...
	// Byte count is conservative vs the UTF-16 CreateProcess limit (bytes ≥
	// UTF-16 units); syscall.EscapeArg quoting overhead is not modeled.
...
(If you create adapter.go fresh with `package agent` + imports as shown, that is correct — the file does not exist yet; just never duplicate `Expand`/placeholder constants from prompts.go.)
...
// use (both stream copiers write it). Returns the exit code; a canceled
// context returns (-1, context.Canceled). Cancel kills the direct child only
// (on Unix, consider Setpgid for whole-tree reaping; Windows is
// direct-child-only); WaitDelay bounds hangs from orphaned descendants that
...
Append missing imports to adapter.go: `bytes`, `context`, `io`, `os`, `time`.
...
// (Added to internal/cli/root.go in this task — apply/refine/diff reuse it.)
