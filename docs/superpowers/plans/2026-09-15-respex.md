...
func TestNewRefusesExistingSpec(t *testing.T) {
	...
	var out bytes.Buffer
	if code := runNew(nil, &out); code != 1 || !strings.Contains(out.String(), "SPEC.md") {
		t.Fatalf("runNew = %d, out=%s", code, out.String())
	}
}
...
func runNew(args []string, out io.Writer) int {
	if len(args) > 1 {
		return fail(fmt.Errorf("usage: respex new [description]"))
	}
	desc := ""
	if len(args) == 1 {
		desc = args[0]
	...
	if _, err := os.Stat(filepath.Join(".respex", "state.db")); err == nil {
		return fail(fmt.Errorf("this directory is already a respex project (.respex/state.db exists)"))
	}
	cfg, err := loadConfig(".")
	if err != nil {
		return fail(err)
	...
	if _, err := os.Stat(specPath); err == nil {
		return fail(fmt.Errorf("%s already exists", cfg.Spec))
	}
	...
	case desc != "" && len(cfg.Agent.Command) > 0:
		if code := draftSpec(cfg, desc, cfg.Spec, out); code != 0 {
			return code
		}
...
func draftSpec(cfg config.Config, desc, specRel string, out io.Writer) int {
	...
	if code != 0 {
		return fail(fmt.Errorf("draft failed (exit %d) — log: %s", code, logPath))
	}
	if _, err := os.Stat(specRel); err != nil {
		return fail(fmt.Errorf("agent did not create %s — log: %s", specRel, logPath))
	}
...
func TestCommitVersions(t *testing.T) {
	...
	if code := runCommit([]string{"-m", "first"}, &out); code != 0 {
	...
	writeSpec(t, root, "# one\n\n## more\n")
	out.Reset()
	if code := runCommit(nil, &out); code != 0 || !strings.Contains(out.String(), "committed v2") {
...
func TestCommitIdenticalWarns(t *testing.T) {
	...
	writeSpec(t, root, "# one\n")
	var out bytes.Buffer
	if code := runCommit(nil, &out); code != 0 {
		t.Fatal("first commit failed")
	}
	out.Reset()
	if code := runCommit(nil, &out); code != 0 || !strings.Contains(out.String(), "unchanged") {
...
func TestCommitMissingSpec(t *testing.T) {
	setupProject(t)
	var out bytes.Buffer
	if code := runCommit(nil, &out); code != 1 {
...
func runCommit(args []string, out io.Writer) int {
...
func TestDiffWorkingVsLastCommit(t *testing.T) {
	root := setupProject(t)
	writeSpec(t, root, "# one\n")
	runCommit(nil, &bytes.Buffer{})
	writeSpec(t, root, "# one\n\ntwo\n")
	var out bytes.Buffer
	if code := runDiff(nil, &out); code != 0 {
...
func TestDiffTwoVersions(t *testing.T) {
	...
	runCommit(nil, &bytes.Buffer{})
	var out bytes.Buffer
	if code := runDiff([]string{"1", "2"}, &out); code != 0 || !strings.Contains(out.String(), "+two") {
...
func TestDiffNoCommits(t *testing.T) {
	...
	writeSpec(t, root, "# one\n")
	var out bytes.Buffer
	if code := runDiff(nil, &out); code != 1 {
...
func TestDiffBadArgs(t *testing.T) {
	root := setupProject(t)
	var out bytes.Buffer
	if code := runDiff([]string{"1"}, &out); code != 1 {
...
func runDiff(args []string, out io.Writer) int {
	w, err := discover()
	if err != nil {
		return fail(err)
	}
	...
	case 0:
		...
		if last == nil {
			return fail(fmt.Errorf("no committed versions yet"))
		}
	...
	case 2:
		...
		if errA != nil || errB != nil {
			return fail(fmt.Errorf("usage: respex diff [vA vB] — version ids are integers"))
		}
	...
	default:
		return fail(fmt.Errorf("usage: respex diff [vA vB]"))
	}
...
func TestApplyDirtySpecFails(t *testing.T) {
	...
	writeSpec(t, root, "# one\nchanged\n")
	var out bytes.Buffer
	if code := runApply(nil, &out); code != 1 || !strings.Contains(out.String(), "spec changed since last commit") {
...
func TestApplyRequiresAgent(t *testing.T) {
	...
	runCommit(nil, &bytes.Buffer{})
	var out bytes.Buffer
	if code := runApply(nil, &out); code != 1 || !strings.Contains(out.String(), "no agent configured") {
...
func TestApplyNoCommits(t *testing.T) {
	...
	writeSpec(t, root, "# one\n")
	var out bytes.Buffer
	if code := runApply(nil, &out); code != 1 || !strings.Contains(out.String(), "no committed versions yet") {
...
func runApply(args []string, out io.Writer) int {
	...
	if last == nil {
		return fail(fmt.Errorf("no committed versions yet — run `respex commit`"))
	}
	...
	if *oneOff != "" {
		...
		if err := toml.Unmarshal([]byte("command = "+*oneOff), &parsed); err != nil || len(parsed.Command) == 0 {
			return fail(fmt.Errorf("invalid --agent template %q", *oneOff))
		}
		tpl = parsed.Command
	}
	if len(tpl) == 0 {
		return fail(errors.New("no agent configured — set [agent] command in .respex/config.toml"))
	}
	delivery := w.cfg.Agent.Delivery
	if delivery == "" {
		delivery = agent.DeliveryArgv
	...
	if code != 0 {
		return fail(fmt.Errorf("apply v%d failed (exit %d) — log: %s", last.ID, code, logRel))
	}
...
`, fakeBin, marker))
	var out bytes.Buffer
	if code := runRefine(nil, &out); code != 0 || !strings.Contains(out.String(), "spec updated") {
...
`, fakeBin))
	var out bytes.Buffer
	if code := runRefine(nil, &out); code != 0 || !strings.Contains(out.String(), "spec unchanged") {
...
func TestRefineNoAgent(t *testing.T) {
	...
	writeSpec(t, root, "# x\n")
	var out bytes.Buffer
	if code := runRefine(nil, &out); code != 1 {
...
func runRefine(args []string, out io.Writer) int {
	w, err := discover()
	if err != nil {
		return fail(err)
	}
	...
	if code != 0 {
		return fail(fmt.Errorf("refine failed (exit %d) — log: %s", code, logPath))
	}
	after, err := spec.Read(w.specPath())
	if err != nil {
		return fail(fmt.Errorf("agent removed the spec — log: %s: %w", logPath, err))
	}
...
func TestLogOutput(t *testing.T) {
	root := setupProject(t)
	writeSpec(t, root, "# one\n")
	runCommit([]string{"-m", "first"}, io.Discard)
	var out bytes.Buffer
	if code := runLog(nil, &out); code != 0 || !strings.Contains(out.String(), "v1") ||
...
func runLog(args []string, out io.Writer) int {
	w, err := discover()
	if err != nil {
		return fail(err)
	}
